...
from src.retrogear.engine.racing_segment import RacingSegment, SubRacingSegment
...
class RacingTrack():
    ...
    def add_racing_segment(self, racing_segment: RacingSegment):
        self.racing_segments.append(racing_segment)

    def get_racing_segments(self) -> List[RacingSegment]: 
        return self.racing_segments
    
    def get_racing_sub_segment(self, distance) -> SubRacingSegment:
        return self.racing_track[math.floor(distance / env.DISTANCE_PER_LENGTH) % len(self.racing_track)]

    def get_racing_track(self) -> List[SubRacingSegment]:
    ...
    def generate_racing_track(self):
        ...
        for i in range(len(self.racing_segments) - 1):
            ...
            # cria uma cópia do RacingSegment do segmento atual como SubRacingSegment
            self.racing_track.append(SubRacingSegment.from_parent(current_segment))
        ...
        self.racing_track.append(SubRacingSegment.from_parent(self.racing_segments[-1]))
